...
if (!function_exists('toTitleCase')) {
  function toTitleCase($str, $options = null)
  {
    ...
    try {
      ...
      $mergedOptions = isset($options['style']) && $options['style'] === 'chicago' ?
      ...
      $capitalizeWord = function ($word) use ($mergedOptions, $uppercaseRegex, $abbreviationRegex) {
        // Handle short conjunctions, articles, and prepositions
        if (in_array($word, $mergedOptions['shortConjunctions']) || in_array($word, $mergedOptions['articles']) || in_array($word, $mergedOptions['shortPrepositions'])) {
          ...
          if ($nextWordIndex < count($words)) {
            $words[$nextWordIndex] = ucfirst($words[$nextWordIndex]);
          }
          return ucfirst($word);
          // Handle hyphenated words
        } elseif (strpos($word, '-') !== false) {
          $hyphenatedWord = explode('-', $word);
          $capitalizedHyphenatedWord = array_map(function ($hyphenatedWord, $index) use ($uppercaseRegex, $abbreviationRegex) {
            if ($index === count($hyphenatedWord) - 1 && preg_match('/^(IV|VI{0,3}|IX|XI{0,3}|XIV|XV|XVI{0,2}|XVII|XIX|[IVX]+)$/i', $hyphenatedWord)) {
              return strtoupper($hyphenatedWord);
            } else {
              return ucfirst(strtolower($hyphenatedWord));
            }
          }, $hyphenatedWord, array_keys($hyphenatedWord));
          return implode('-', $capitalizedHyphenatedWord);
          // Handle all other words
        } else {
          if (preg_match($uppercaseRegex, substr($word, 1)) || preg_match($abbreviationRegex, substr($word, 1))) {
            return $word;
          } else if (in_array($word, $mergedOptions['neverCapitalized'], true)) {
            return strtolower($word);
          } else {
      ...
      $words = explode(' ', $str);
      $capitalizedWords = array_map(function ($word, $index) use ($capitalizeWord, $words, $mergedOptions) {
