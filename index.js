...
String.prototype.toTitleCase = function(options) {
    try {
        ...
        const capitalizeWord = (word) => {
            ...
            if (mergedOptions.neverCapitalized.includes(lowerWord) || mergedOptions.shortConjunctions.includes(lowerWord) ||
                mergedOptions.articles.includes(lowerWord) || mergedOptions.shortPrepositions.includes(lowerWord)) {
        ...
        const checkColonWords = function(str) {
            const words = str.split(" ");
            const colonWords = [];
            const lowerWords = ['a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor', 'on', 'at', 'to', 'from', 'by', 'in', 'of', 'with', 'up', 'as', 'yet', 'so'];
            ...
            for (let i = 0; i < words.length - 1; i++) {
                ...
                if (word.includes(":") && nextWord) {
                    const after = nextWord.toLowerCase();

                    if (lowerWords.includes(after)) {
                        colonWords.push({
                            before: word.replace(":", ""),
                            after: after
                        });
        ...
        const capitalizedWords = words.map((word, index) => {
            ...
            } else if (uppercaseRegex.test(word)) {
                return word;
            } else {
