...
const CORRECTED_TITLE_CASE_TERMS = {
  'PhD': 'ph.d.',
  'F.Y.I': 'FYI',
  'T.B.D': 'TBD',
  'A.K.A': 'AKA',
...
function isPhraseIgnored(words, IGNORED_TITLE_CASE_PHRASES) {
  for (let i = 0; i < words.length; i++) {
    ...
    const ignorePhrase = IGNORED_TITLE_CASE_PHRASES.find(phrase => phrase.toLowerCase() === currentWord.toLowerCase() || phrase.toLowerCase().startsWith(currentWord.toLowerCase() + ' '));
    if (ignorePhrase) {
...
function processHyphenatedWord(word, style) {
  ...
  const processedWords = hyphenatedWords.map((hyphenatedWord, j) => {
    if (isShortConjunction(hyphenatedWord, style) || isArticle(hyphenatedWord, style) || isShortPreposition(hyphenatedWord, style) || isNeverCapitalized(hyphenatedWord, style)) {
  ...
  try {
    ...
    const result = words.map((word, i) => {
      ...
      if (isIncorrectTerm(word, Object.keys(CORRECTED_TITLE_CASE_TERMS))) {
        const correctWordIndex = Object.keys(CORRECTED_TITLE_CASE_TERMS).findIndex(w => w.toLowerCase() === word.toLowerCase());
        const correctedTerm = Object.values(CORRECTED_TITLE_CASE_TERMS)[correctWordIndex];
        return correctedTerm;
      }
      ...
      if (isShortConjunction(word, style) || isArticle(word, style) || isShortPreposition(word, style) || isNeverCapitalized(word, style)) {
